...
## QUALITY_CHECKLIST
    ...
    // Check if this is a consistency result (JSON object) or a fetch response
    if (openaiResponse && typeof openaiResponse === 'object' && !openaiResponse.ok && !openaiResponse.status) {
      // This is a consistency result, not a fetch response - skip error checking
      console.log('✅ Using consistency result, skipping response validation')
    } else if (openaiResponse && !openaiResponse.ok) {
    ...
    if (openaiResponse && typeof openaiResponse === 'object' && !openaiResponse.ok && !openaiResponse.status) {
      // This is a consistency result, convert to streaming response
      ...
      // This is a fetch response, return as-is
...
Be helpful, accurate, and efficient. Focus on delivering value without BS.`
    ...
    // Check if this is a consistency result (JSON object) or a fetch response
    if (openaiResponse && typeof openaiResponse === 'object' && !openaiResponse.ok && !openaiResponse.status) {
      // This is a consistency result, extract response directly
      console.log('✅ Using consistency result for non-streaming')
      const responseText = openaiResponse.choices?.[0]?.message?.content || 
                          openaiResponse.message || 
                          'I apologize, but I was unable to generate a response.'
      ...
      if (canPersist && responseText.length > 50) {
        try {
          ...
          console.log('🔍 SemanticCache model available:', !!prisma.semanticCache)
    ...
    if (canPersist && responseText.length > 50) {
      try {
        console.log('🔍 Attempting to cache response (fallback)...')
        console.log('🔍 Prisma client available:', !!prisma)
        console.log('🔍 SemanticCache model available:', !!prisma.semanticCache)
        await prisma.semanticCache.create({
          data: {
