          ...
          <div className="flex-1 min-w-0">
            {item.kind === 'site' && item.site ? (
              <>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{item.site.niche}</p>
                <div className="flex items-center space-x-2 mt-1">
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">${(item.site.publishing.price || 0).toFixed(2)}</span>
                  <span className="text-sm text-gray-500 dark:text-gray-400">× {item.quantity}</span>
                </div>
              </>
            ) : (
              <>
                <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{item.product?.name}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 truncate">Package</p>
                <div className="flex items-center space-x-2 mt-1">
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">${(item.product?.priceDollars || 0).toFixed(2)}</span>
                  <span className="text-sm text-gray-500 dark:text-gray-400">× {item.quantity}</span>
                </div>
              </>
            )}
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-lg font-bold text-gray-900 dark:text-gray-100">
            ${state.items.reduce((total, item) => {
              if (item.kind === 'site' && item.site) return total + (item.site.publishing.price || 0) * item.quantity
              if (item.kind === 'product' && item.product) return total + (item.product.priceDollars || 0) * item.quantity
              return total
            }, 0).toFixed(2)}
          </span>
        </div>
