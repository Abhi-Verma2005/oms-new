...
export function ChatActionCard({ card, onDismiss, className }: ChatActionCardProps) {
  ...
  return (
    <div
      className={cn(
        'w-full bg-white/5 border border-white/10 rounded-[8px] p-3 text-white/90 transition-all duration-150 ease-out',
    ...
    >
      <div className="flex items-start gap-2">
        {card.icon ? (
          <div className="mt-0.5 h-4 w-4 text-[#cccccc]">{card.icon}</div>
        ) : null}
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="text-[13px] font-medium leading-5 truncate">{card.title}</div>
            {card.dismissible !== false && (
              <button
                aria-label="Dismiss"
                onClick={handleDismiss}
                className="shrink-0 h-5 w-5 inline-flex items-center justify-center rounded-[4px] text-[#858585] hover:text-[#cccccc] hover:bg-[#2d2d30] transition-all duration-150"
              >
          ...
          {card.message ? (
            <div className="text-[12px] text-white/60 mt-0.5 leading-5">{card.message}</div>
          ) : null}
          ...
          {isFilterCard && showRefine && (
            <div className="mt-3 rounded-[8px] border border-white/10 bg-white/5 p-3">
              <div className="text-[12px] text-white/70 mb-2">Set price range</div>
              <RangeSelector
                min={0}
                max={5000}
